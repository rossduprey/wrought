...
```
cd core
make test          # 110 assertions. Must print "ok (0 failures)".
...
**Copper is now built** (2026-07-10) — the chain generalizes: the same ledger, the same anvil, a different reduction that changes the metal's *state* at the furnace. The natural copper follow-ons, none started: **bronze** (alloy copper with tin — tin lowers the melting point and hardens the metal, so it touches both the furnace and the hardness law, and it is the actual Bronze-Age payoff), **sulfide copper** (chalcopyrite/chalcocite need a *roast* to drive off sulfur before they will smelt — the copper cousin of red-short, but caught at the furnace instead of the anvil), and **tin itself** as its own reducible metal. Everything else open is a set piece (**#15**, deflocculation), an unbuilt model (**#19**, hiding–exposure), or a citation chore (**#1, #2, #5, #7, #12, #16, #17, #18, #20, #21, #22, #23, #24, #25**). This session's debt is **#25**: copper's reduction-onset temperature (`CU_REDUCTION_T`, a cited envelope) and its reuse of iron's Brinell hardness *scale* (`H_ANNEALED`/`H_SATURATED`) are authored — neither can reverse the finding they sit under (*abundance is not the gate, melting point is*), which rides the furnace heat sitting between copper's and iron's melting points, both verified, not the rates.
