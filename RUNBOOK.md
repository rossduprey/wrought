...
```
cd core
make test          # 91 assertions. Must print "ok (0 failures)".
...
| `forge.h` | The forge. The first **deformation** process — it squeezes a porous solid instead of sorting grains. A bloom is a slag-filled sponge; consolidation drains slag only through connected pores, so below a percolation floor the rest is trapped as stringers (~1.5%, connectivity not effort). Every heat also scales iron away, so a cleaner bar is a smaller one — the grade/recovery law a fourth time. Two floors derived (geometry), the drain/scale *rates* authored (#22). |
...
Era 0 (hands, pan, sluice, lodestone) and Era 1 (levigation, fired pot) are implemented and measured, and Era 2 is underway: the bloomery smelts and iron falls out of it (`smelt.h`), and the forge consolidates that bloom into a bar (`forge.h`, both 2026-07-10). The iron chain now runs dig → pan → crush → lodestone → bloomery → forge, end to end. What does not exist yet: **shaping** (bar → a specific tool, with work-hardening), and copper.
...
The next piece of work is **bar → tool**: *shaping*, the second half of the forge. Consolidation is deformation that only densifies; shaping is deformation that gives a form and work-hardens it, and it is where pyrite finally bites — the red-short bar that cracks under the hammer because the sand held a sulfide grain nobody could see (`README.md` claim #2). It is also the axis copper *shares* with iron, so it is the natural place to open the copper branch afterward. Everything else open is a set piece (**#15**, deflocculation), an unbuilt model (**#19**, hiding–exposure), or a citation chore (**#1, #2, #5, #7, #12, #16, #17, #18, #20, #21, #22**). This session's debt is **#22**: the forge's slag-drain and iron-scale *rates* are authored, as is the green sponge's packing and the connectivity floor — none of which can reverse the two findings they sit under (the stringer floor and the fourth grade/recovery law), which ride the rates' signs, not their values.
