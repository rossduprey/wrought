...
```
cd core
make test          # 96 assertions. Must print "ok (0 failures)".
...
The next piece of work is **red-short**: the sulfur-bearing bar that cracks when it is worked hot, `README.md` claim #2 — *the sand held a pyrite grain nobody could see*. This is where pyrite finally bites, and it needs a piece of plumbing first: **sulfur is not on the element ledger.** Pyrite's sulfur lumps into `EL_OTHER`, which `bloomery()` routes entirely into slag (`smelt.h:184`), so today's bloom carries no sulfur and there is nothing to crack. Red-short means adding `EL_S` to the `Element` enum, splitting pyrite's sulfur out of `EL_OTHER`, and partitioning a fraction of it into the *metal* (an authored partition coefficient, its own issue) — then the crack rides on the shaping already built: worked above the Fe–FeS eutectic film's melting point (~985 °C, a *derived* threshold like the fayalite wall), a sulfur-bearing bar comes apart at the grain boundaries. It is a bigger change than shaping was, because it touches the smelt ledger, not just the anvil. Everything else open is a set piece (**#15**, deflocculation), an unbuilt model (**#19**, hiding–exposure), or a citation chore (**#1, #2, #5, #7, #12, #16, #17, #18, #20, #21, #22, #23**). This session's debt is **#23**: the shaping half's recrystallization fraction, hardness scale, and hardening rate are authored — none can reverse the finding they sit under (*form free, hardness only cold*), which rides the recryst floor's existence and hardening's monotone-saturating shape, not the values.
