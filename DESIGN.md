...
1. **Depth is a tier, not a carved z-axis — and it changes the *mineral*, not just
   ...
   **The gesture, decided and now built** (`core/dig.cpp`, the second playable
   ...
   cutting**, and strikes the ground with the **hand or the pick** — one strike,
   ...
   same limit the pan hit with its swirl). The pick is the Era-0→1 rung (it *wins*
   rock from the face; the shovel is the loader downstream): it takes ~10× the bite
   and drives the pit down faster, which is why a pick with no cart just buries you. Nothing is stated; the choice — a small clean oxide pile up top
...
2. **A full-depth dig MIXES the column — and it comes up as a *size distribution*,
   free fines over locked coarse.**
   ...
   fires. A pick blow does not yield one coarse grade: it shatters rock across sizes,
   ...
   > *This paragraph said, until 2026-07-11, that the dig "all comes up as **locked
   > rock** — nothing in that pile is a clean grain," so the whole heap must reach the
   > breaker before anything is winnable. Monitored play refuted it. A pick (not a
   > **shovel** — a shovel MOVES loosened rock, a pick WINS it from the face; the file
   > conflated the two) breaks rock across a **particle-size distribution**, and the
   > fine end of a placer's spoil is already-free grains a pan takes with no crusher.
   > `geology.h`'s `sample()` now spreads the won rock over a Gates-Gaudin-Schuhmann
   > PSD and splits each size against the mineral's **liberation size** (`liberation_
   > bin()`): cassiterite frees at gravel (placer, coarse), cuprite at sand (weathered
   > oxide), chalcocite only at clay (~1 µm dissemination) — sourced grain sizes, not
   > game knobs. The earlier #28 "shortcut" that was rightly killed was liberation
   > **independent of size** (all ore free, trivializing the crusher); the truth is
   > liberation **as a function of size**, which is not a shortcut but the whole
   > premise of `substance.h` ("magnetite is free in the sand and locked in the
...
*(Also built 2026-07-10: **tin and bronze**, `smelt_tin()` + `alloy()`. Two notes for this plan. **First, the alloy's finding is a single cause read twice.** Bronze was easy to pitch as "harder and lower-melting," two facts; the build makes them one — the misfit of the tin atom in the copper lattice both strains the solid (solid-solution strengthening lifts the hardness curve) and disorders the freeze (melting-point depression), so `alloy()` sets exactly one number, the tin fraction, and both properties fall out of it. The code demonstrates "one law, moved inputs" literally: bronze is cast, stocked, drawn and hardened by the very same functions as pure copper, which now read `Tool.h_annealed`/`h_saturated` and `Tool.tm` instead of globals — pure metals keep the defaults, so nothing upstream moved. **Second, the tin finding has a half the code cannot say.** Tin is trivial to smelt and the model says so honestly (no wall, no trap, always a pour); but the historically decisive fact — tin is *rare and localized*, so bronze forced trade — is geology, and there is no map in this codebase. "Co-location is the gate" is therefore prose, not a mechanic, and it joins cobbing on the list of things that would need an ore-distribution/geography model to become real. The bonus result that cold-worked bronze out-hardens cold-worked wrought iron — why iron did not at once retire bronze — rides the shared, simplified Brinell scale and is a comparison of signs, not a claim about absolute hardness.)*

*(Also built 2026-07-10: **sulfide copper**, `roast()`. Two notes for this plan. **First, it made the sulfur story a matched pair, and that pairing is the finding — not the roast chemistry.** The plan already had red-short (iron's sulfur, invisible at the furnace, incurable at the anvil); the roast is its exact inverse (copper's sulfur, loud at the furnace, curable there), and the build was worth doing precisely because the *contrast* is a stronger claim than either half alone: same villain, two verdicts, and the reason is a property difference — copper's sulfide roasts cleanly to a reducible oxide, iron's contaminant does not. The mechanism is modelled as what it physically is: a reducing fire strips oxygen and a sulfide has none, so the roast must run the *opposite* atmosphere first, adding oxygen (from air, outside the charge ledger, exactly as the bloomery blast) so a later fire can remove it. Two fires, opposite atmosphere, fixed order; the test proves neither alone yields metal. **Second, the honest simplifications.** Only **chalcocite (Cu₂S)** is modelled, not chalcopyrite (CuFeS₂) — chalcopyrite drags iron and the whole matte-smelting cycle, which is genuinely post-Bronze-Age and a step past this plan's ceiling; chalcocite isolates the sulfur story cleanly. And **one roast is idealized to drive off *all* the sulfur** — real roasts stall at a copper sulfate below heat and leave a residual matte that needs repeated roast-reduce cycles. Both are tracked (#27). Atmosphere here is still not a first-class state variable: `roast()` and `smelt_copper()` are two functions whose opposite operations *embody* the two atmospheres rather than reading an `atmosphere` field — which is enough for the finding but is the thing to generalize if reverberatory/converter chemistry ever gets built.)*
