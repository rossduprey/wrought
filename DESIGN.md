...
1. **Depth is a tier, and it changes the mineral.** A copper body wears a weathered oxide cap (cuprite, smelts straight) over a leached middle over a sulfide root (chalcocite, must be roasted — Era 2b). The dig bar crosses these tiers; the assay panel shifts live with depth. The pick is the Era-0→1 rung, and it does two things. *Rate:* it wins rock from the face (~10× the bite), driving the pit down faster. *Reach:* depth is also a **rising wall**, not only a changing mineral — weathering left the oxide cap friable enough for bare hands, but the fresh sulfide root is competent rock a hand skips off, and only the pick's concentrated blow (its `haft.h` `bite()`, force not sharpness) gets under it. So the primary ore — and roast-grade copper — is gated behind the whole tool bootstrap, while a placer the river already broke needs no pick at all (`rock_competence`, on the same arbitrary energy scale as `swing_energy`; #39). *The gossans were worked for an age before the deep sulfides* stops being prose and becomes a fact about which tool you are holding. On a tier both tools can win, the makeup is identical (`sample()` is tool-blind) — the tool changed reach and rate, never makeup.
...
The bootstrap turns on **one edge hafted twice, not one tool** — with two *edgeless* stations between the two haftings. A felled trunk is a green log, not a haft, and it passes four verbs before a tool rides it. Felling is **force** (an axe, `EDGE_ACROSS`, severs the standing grain; the sapling swing is enough because its bite tops the fell wall). Then come the two verbs that need no edge at all. **Splitting** (`split.h`) rives the round log into billets with a *wedge* — a froe or a hardwood glut and a mallet — that opens a crack the grain runs; the gate is the grain (along it wins a full-length-fibred billet, across it wins short-grained waste), not force or sharpness, so it can be done before the adze even exists. **Seasoning** (`season.h`) is the first station gated by the *calendar*: a green billet is three-quarters water, and only time dries it. Water leaves by Fickian diffusion, so drying time grows with the **square** of the billet's thickness (a thin riven billet air-dries in a season; the whole log takes a lifetime — which is *why* you split thin first). Nothing stiffens until the wood falls below fibre saturation, and a head bound green works loose as the wood shrinks — the joint fails at the calendar, not the swing. Only then does **dressing** (`dress.h`) return the edge: the adze, `EDGE_INLINE`, pares *along* the grain to true the seat the round riven billet lacks. `dress(stave, adze)` yields `TIMBER` only for a billet BOTH riven along the grain AND seasoned to air-dry — miss either and it hands back the `SAPLING`, so seasoning is what *earns* the full timber joint the springy sapling never could seat. The stone adze leaves a face only as true as knap's edge floor (metal dresses finer); true *enough* is the whole bar for a haft, so the floor caps the finish, not the hold. Knap once, haft twice, a wedge and a calendar between: hands → stone edge → sapling **axe** → the felled trunk → a riven billet → a seasoned billet → the **adze**-trued timber haft → every tool after.
