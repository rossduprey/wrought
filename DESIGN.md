...
The bootstrap turns on **one edge hafted twice, not one tool** (`dress.h`). A felled trunk is a green log, not a haft: the axe *wins* the wood, but turning that log into a stiff true handle is a second verb — rive along the grain, pare a face true — and it wants the opposite geometry of the axe. Felling is **force** (an axe, `EDGE_ACROSS`, severs the standing grain; the sapling swing is enough because its bite tops the fell wall). Dressing is the **edge** (an adze, `EDGE_INLINE`, pares *along* the grain in controlled bites, no fell-wall behind it). So the axe that felled the trunk cannot dress it, and the same crude sapling kit that felled the log also dresses it — force needs the lever, the edge does not, which is the only reason the first haft can exist before there is any timber to make one from. The stone adze leaves a face only as true as knap's edge floor (metal dresses finer); true *enough* is the whole bar for a haft, so the floor caps the finish, not the hold. Knap once, haft twice: hands → stone edge → sapling **axe** → the felled trunk → the **adze** → a seasoned timber haft → every tool after.
