...
- **Core (`core/`, header-only):** phase table, substance, settling, separation (Tromp), magnetic, smelt (bloomery + copper/tin furnace + roast), forge (consolidate + shape + cast + alloy), levigate (+ flocculate), fire, geology (spatial field; the pick gate — the deep rock is a competence wall only a concentrated blow gets under), fuel (wood → pyrolysis → charcoal; the two-gate finding; fire tiers), char (the charcoal-pit transform; the seal is the gate), knap (stone → edge; fracture is the gate; the edge's irreducible floor), haft (edge + stick → the first tool; the haft is force, the joint is the ceiling), dress (felled trunk → the timber haft; the axe wins the trunk, the adze makes the haft — one edge hafted twice).
...
- A felled trunk is not a haft, and the axe that felled it cannot make it one. Felling is force (axe, `EDGE_ACROSS`, severs the standing grain — the sapling swing is enough); dressing is the edge (adze, `EDGE_INLINE`, pares along the grain, no fell-wall behind it). So the bootstrap turns on one knapped edge hafted TWICE — axe to win the trunk, adze to dress it into `TIMBER` — and the same crude sapling kit that felled the log also dresses it, because the edge dresses where the lever fells. `dress.h` authors no number: the transform is structural (adze-along-grain wins a haft, nothing else does), the finish capped by knap's edge floor (metal dresses finer). Closes the loop haft.h left as a freebie — felling no longer hands you finished timber; you dress it.
...
- **The fuel/tool staircase** (all built): knapping (`knap.h` + `knap` slice); hafting (`haft.h` + `haft` slice); dressing (`dress.h`, the adze verb wired into the `haft` slice — felling yields a raw log, the adze dresses it to timber); the pick gate (`geology.h` `rock_competence` + `dig` slice) closes the loop back to the dig. Staircase runs end to end in the model: hands → stone edge → sapling axe → felled trunk → adze → real timber haft → pick → the deep sulfide root the bare hand could not win. `dress.h` authors no number (structural gate); no new citation issue.
