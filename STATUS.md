...
- **Unbuilt rungs** (the fuel/tool staircase, model in `fuel.h`, stations/verbs pending): knapping (stone → edge); hafting (edge + head → pick/axe); wiring gathered/pit char into `furnace.cpp` to retire the free `charcoal_bed()`.
